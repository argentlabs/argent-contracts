...
const main = async () => {
  ...
  for (const vault of config.defi.yearnV2.pools) {
    await installFilter({
      filterDeployer: async () => getFilterFromConfigOrDeployNew(YearnV2Filter),
      dapp: vault,
      dappName: `yVault ${vault}`,
      filterName: "YearnV2Filter"
    });
  }
};
