...
describe('Test Kyber', function () {
    ...
    it('should return the expected rate for a token pair', async function() {
        let rate = await kyber.getExpectedRate(erc20.contractAddress, ETH_TOKEN, 1);
        assert.equal(rate[0], ERC20_RATE, "rate should be correct");
    });

    it('should exchange ETH for ERC20', async function() {
        let beforeERC20 = await erc20.balanceOf(trader.address);
        let beforeETH = await deployer.provider.getBalance(trader.address);
        ...
        let afterERC20 = await erc20.balanceOf(trader.address);
        let afterETH = await deployer.provider.getBalance(trader.address);
    ...
    it('should exchange ERC20 for ETH', async function() {
        // provision ERC20 to trader
        await kyber.trade(ETH_TOKEN, ethers.utils.bigNumberify('1000000000000000000'), erc20.contractAddress, trader.address, ethers.utils.bigNumberify('10000000000000000000000'), 1, '0x0000000000000000000000000000000000000000', {value: 1000000000000000000, gasLimit: 200000});
        let beforeERC20 = await erc20.balanceOf(trader.address);
        let beforeETH = await deployer.provider.getBalance(trader.address);
        assert.equal(beforeERC20 > 0, true, "trader should have some ERC20");
        // exchange ERC20
        let srcAmount = beforeERC20.div(ethers.utils.bigNumberify(2));
