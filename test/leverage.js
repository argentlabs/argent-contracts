...
describe("Test Leverage Module", function () {
    ...
    let deployer, cdpManager, leverageManager, wallet, sai, gov, tub, kyber, exchanger;
    ...
    before(async () => {
        ...
        leverageManager = await deployer.deploy(LeverageManager, {},
            registry.contractAddress,
            guardianStorage.contractAddress,
    ...
    beforeEach(async () => {
        ...
        await wallet.init(owner.address, [exchanger.contractAddress, cdpManager.contractAddress, leverageManager.contractAddress]);
    ...
    describe("Leverage Positions", () => {
        ...
        async function openLeveragedPosition({ collateral, conversionRatio, iterations, relayed }) {
            ...
            const params = [wallet.contractAddress, collateral, conversionRatio, iterations, 0];
            let txReceipt;
            if (relayed) {
            ...
            return {
                ...
                ethExposure: formatEther(log.pethCollateral),
                leverage: formatEther(log.pethCollateral.mul(WAD).div(collateral)),
                daiDebt: formatEther(log.daiDebt),
                liquidationRatio: log.daiDebt.gt(0) && formatEther(log.pethCollateral.mul(WAD).mul(WAD).div(ETH_PER_DAI).div(log.daiDebt)),
        ...
        async function closeLeveragedPosition({ cup, daiRepaid, relayed }) {
            ...
            const params = [wallet.contractAddress, cup, daiRepaid, 0, 0];
            let txReceipt;
            if (relayed) {
                txReceipt = await manager.relay(leverageManager, method, params, wallet, [owner], undefined, false, 4700000);
            } else {
